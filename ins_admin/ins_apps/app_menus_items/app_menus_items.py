...
class AppMenusItems(App):
    ...
    def _menu_data(self):
        ...
        for k in data:
            f= data[k]
            m = ""
            if "_mid" in g and str(g["_mid"]) == str(f["id"]):
                m = "selected ='true' "

            t += f"<option {m} value='{f["id"]}'>{f["title"]}</option>"
            if "sub" in f:
                m =""
                for sub in f["sub"]:
                    if "_mid" in g and g["_mid"] == sub["id"]:
                        m = "selected ='true' "
                    t += f"<option {m} value='{sub["id"]}'>&nbsp;&nbsp;&nbsp;|--&nbsp;{sub["title"]}</option>"
    ...
    def get_pros(self):
        g = self.ins._server._req()
        area_url = self.ins._eng._areas(g["_myarea"])["url"]
        url = f"{self.ins._map.APPS_FOLDER}/{g["_type"]}/"
        url = f"{area_url}/{url}"
        ops = {}
        opsa = {}
        ...
        if g["_mode"] == "edit":
            w = f"id={g["_id"]}"
            ops = self.ins._db._get_disabled()._get_row("kit_menu_item", "kit_options", w)
