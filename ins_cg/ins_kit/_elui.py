...
class ELUI(ins_parent):
    ...
    def cart_pro_block(self,data,string=False):
        uidata = [
            ...
            {"src": f"{self.ins._map.UPLOADS_FOLDER}{data['th_main']}","loading":"lazy", "_type": "img","class": "ins-radius-m", "style": "    height: 100%;"},
            {"end": "true"},
            ...
            {"_data": f"<i  class='lni lni-trash-3 _a_red'></i>","class": "ins-flex-center ins-col-1 ins-m-col-1 -remove-item-side-cart-btn", "data-pid": data["id"]},

            {"_data": f" {data["title"]}", "class": "ins-col-7 ins-m-col-7 ins-strong-m ins-grey-color ins-title-14"},
            {"_data":  f"{data["price"]}","_view":"currency","_currency_symbol":" EGP","_currency_symbol_ar":" جنيه", "class": "ins-col-5 ins-m-col-5 ins-strong-m ins-grey-d-color ins-flex-end ins-title-14"},
            {"end": "true"},
            {"end": "true"}
        ]
        if string:
    ...
    def shop_pro_block(self, data ,url =""):
        ...
        if url =="":
            ...
            url = self.ins._server._url({"alias": "products", "mode": "category", "id" :data['cat_alias'],"q1": f"{data['alias']}"},["page"])
        else:
            url =url
        ...
        r = [
            {"_type": "a", "href":url,"start": "true", "class": "ins-flex ins-card ins-col-3 -pro-item-block","style":""},
